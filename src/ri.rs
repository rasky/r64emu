...
pub struct Ri {
    ...
    // [2] stop T active
    // [3] stop R active
    #[reg(bank = 0, offset = 0x00, rwmask = 0xF)]
    mode: Reg32,

    // [5:0] current control input
    // [6] current control enable
    #[reg(bank = 0, offset = 0x04, rwmask = 0x3F)]
    config: Reg32,

    // (W): [] any write updates current control register
    #[reg(bank = 0, offset = 0x08, writeonly)]
    current_load: Reg32,

    // [2:0] receive select
    // [2:0] transmit select
    #[reg(bank = 0, offset = 0x0C, rwmask = 0xF)]
    select: Reg32,
    ...
    // [17] refresh enable
    // [18] refresh optimize
    #[reg(bank = 0, offset = 0x10, rwmask = 0x7FFFF)]
    refresh: Reg32,

    // [3:0] DMA latency/overlap
    #[reg(bank = 0, offset = 0x14, rwmask = 0xF)]
    latency: Reg32,

    // (R): [0] nack error
    //      [1] ack error
    #[reg(bank = 0, offset = 0x18, rwmask = 0x2, readonly)]
    error: Reg32,

    // (W): [] any write clears all error bits
    #[reg(bank = 0, offset = 0x1C, writeonly)]
    error_write: Reg32,
